...
public class TelaVenda {
	...
	private void initialize() {
		...
		labeltipopagamento.setBounds(256, 217, 100, 14);
		...
		labelvalorpagamento.setBounds(21, 277, 110, 14);
		...
		labelcorretor.setBounds(256, 277, 50, 14);
		...
		labelcliente.setBounds(256, 246, 42, 14);
...
//	 				labelrodape.setText("Atendimento atualizado : "+protocolo);
	...
	public void listagem() {
		try{
			List<Venda> lista = Fachada.listarVenda();
			...
			if (idpesquisa == null) {
				for(Venda v : lista) {
					model.addRow(new Object[]{v.getId()+"", v.tipo, v.tipopagamento, v.valorpagamento, v.getCorretor().getNome(), v.getCliente().getNome()});
				}
			}
			else {
				for(Venda v : lista) {
					if (String.valueOf(v.getId()).contains(idpesquisa)) {
						model.addRow(new Object[]{v.getId()+"", v.tipo, v.tipopagamento, v.valorpagamento, v.getCorretor().getNome(), v.getCliente().getNome()});
					}
				}
			}
